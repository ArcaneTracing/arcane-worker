"""
ResponseGroundness evaluation using RAGAS.
"""
...
class ResponseGroundnessInput(BaseModel):
    """Input type for ResponseGroundness evaluation."""
...
async def evaluate_response_groundness(
    input_data: ResponseGroundnessInput,
    llm: Optional[BaseRagasLLM] = None
) -> RagasScore:
    """
    Evaluate ResponseGroundness metric.
    
    Args:
        input_data: ResponseGroundnessInput containing response and retrieved_contexts
    ...
    """
    if llm is None:
        raise ValueError("LLM is required for ResponseGroundedness metric")
    ...
    return RagasScore(
        score=score,
        metric="ResponseGroundness",
        id=METRIC_IDS["response_groundness"]
    )
